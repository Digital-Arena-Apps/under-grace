...
export default function SettingsPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-ink/30 px-4 py-4 backdrop-blur-sm dark:bg-black/40">
      <aside className="flex h-full w-full max-w-md flex-col rounded-lg border border-white/60 bg-ivory p-5 shadow-calm dark:border-white/10 dark:bg-dusk sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-sage/10 text-cedar dark:bg-white/10 dark:text-shell">
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-sage/20 bg-white/60 text-cedar transition hover:bg-white focus:outline-none focus:ring-2 focus:ring-sage/40 dark:border-white/10 dark:bg-white/10 dark:text-shell dark:hover:bg-white/20"
        ...
        <div className="mt-7 space-y-3 border-t border-sage/20 pt-6 dark:border-white/10">
          ...
          <button
            ...
            className="flex w-full items-center justify-center rounded-lg border border-ember/25 bg-ember/10 px-4 py-3 text-sm font-semibold text-ember transition hover:-translate-y-0.5 hover:bg-ember/20 focus:outline-none focus:ring-2 focus:ring-ember/30"
          >
            Clear daily history
          </button>
        </div>

        <p className="mt-auto pt-6 text-xs leading-5 text-cedar/60 dark:text-shell/50">
...
function SettingToggle({ label, description, checked, onChange }) {
  return (
    <label className="flex cursor-pointer items-start justify-between gap-4 rounded-lg border border-sage/20 bg-white/50 p-4 dark:border-white/10 dark:bg-white/10">
      <span>
        <span className="block font-semibold text-ink dark:text-pearl">{label}</span>
        <span className="mt-1 block text-sm leading-6 text-cedar/70 dark:text-shell/60">{description}</span>
      </span>
      <input
        type="checkbox"
        checked={checked}
